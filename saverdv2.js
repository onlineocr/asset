...
const fileExt = 'PDF files'
const zipFileName = "OnlineOcr.io-Pdf2Txt.zip"
const pleaseUploadMsg = 'Please upload PDF files.'
const singleFileOcrMsg = `<strong>Language Selection:</strong><br>
                              Please choose the language of the text your PDF contains.  
                              ...
                              If your PDF contain different languages, you can select a language for each PDF individually.  
                              If all PDF contain text in the same language, you can select it once for all.`;
const OcrCompleteMsg = `You can view the extracted text for each PDF and download its corresponding text file by clicking 
...
const genMsgAfterOcr = 'You can view the extracted text for each PDF and download its corresponding text file by clicking <strong class="badge text-bg-secondary">View Text</strong>. To save all text files at once, simply click the button below';
...
pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdn.jsdelivr.net/gh/onlineocr/asset/pdf.worker.min.js';
...
function setupEventListeners() {
    ...
    document.getElementById('ArePDFsSame').addEventListener('change', function () {
    ...
    // OCR button handler
    $('#_OcrClick').on('click', processAllPDFs);
...
async function handleFileUpload(inputElement) {
    ...
    $('#_pdf-thumbnails').html(`
        <div id="loading_initial" class="col col-auto colas d-flex justify-content-center _pdfCard"><div><figure class="figure p-3 alert alert-warning border-dashed _pdfCard rounded pb-4 pt-4 justify-content-center" style="max-width: 160px;height: -webkit-fill-available;">Please wait while we load your <strong class="fw-bold">PDF</strong> files... <div role="status" class="spinner-border text-dark spinner-border-sm"></div></figure></div></div>
      `);
...
async function processPDFFile(file, index) {
    return new Promise((resolve) => {
        const reader = new FileReader();
        reader.onload = async function (e) {
            const arrayBuffer = e.target.result;
            const imgData = await generatePDFThumbnail(arrayBuffer, 1);
            createPDFThumbnailCard(index, file.name, file.size, imgData[0].image, '_pdf-thumbnails');
            resolve();
        };
...
async function generatePDFThumbnail(pdfData, pageNumber = null) {
    const loadingTask = pdfjsLib.getDocument({ data: pdfData });
    const pdf = await loadingTask.promise;
    const _PdfPageCount = pdf.numPages;
...
// Create PDF thumbnail card
function createPDFThumbnailCard(fileId, fileName, fileSize, imageUrl, parentElementId) {
    const parentElement = document.getElementById(parentElementId);
    if (!parentElement) return;

    // Create main container
    const colDiv = document.createElement('div');
    colDiv.className = `col col-auto colas d-flex justify-content-center _pdfCard del-f-${fileId}`;
    colDiv.dataset.fi = fileId;

    // Create options header
    const optionsSpan = document.createElement('span');
    optionsSpan.className = '_PdfCardHeader';

    // Add delete button
    const deleteSpan = document.createElement('span');
    deleteSpan.className = 'text-danger cursor-pointer del-f fw-bold';
    deleteSpan.title = 'Remove this pdf from list';
    ...
    viewPagesSpan.title = 'View text of this pdf';
    ...
    langSelectSpan.title = 'Select the language of the text used in this PDF.';
    ...
    figure.className = 'figure p-3 alert alert-secondary border-dashed _pdfCard rounded pb-4 pt-4 justify-content-center';
    ...
    imgElement.title = 'View first page of this PDF';
    imgElement.classList.add('_ViewPdfImage');
...
function setupThumbnailEventHandlers() {
    ...
    $('body').on('click', '.sendtowa', function () {
        let fileID = $(this).attr('fi');
        let textarea = document.getElementById(`_PageDetailBody1${fileID}`);
        let textContent = textarea.value;
        let site = 'This text was extracted from a PDF using OnlineOCR.io (https://onlineocr.io/pdf-to-text):\n\n'
        let encodedMessage = encodeURIComponent(site + textContent);
...
function showSidebarWhenReady(filesCount) {
    const interval = setInterval(function () {
        ...
        if ($('#_pdf-thumbnails').length && count === filesCount + 1) {
    ...
    return new Promise((resolve, reject) => {
        ...
        fileReader.onload = async function () {
            try {
                ...
                await extractTextFromPDF(typedArray, fileID, file, _selectedLang);
...
async function extractTextFromPDF(typedArray, fileID, file, _selectedLang) {
    const pdf = await pdfjsLib.getDocument(typedArray).promise;
    $('#_plzWaitOcr').html('<b class="fw-bold">Processing PDF: </b>' + file.name + ' ');
    totPage = pdf.numPages;
    ...
    for (let i = 1; i <= pdf.numPages; i += batchSize) {
        ...
        const batchText = await processPageBatch(pdf, i, end, _selectedLang, fileID, file);
        extractedText += batchText;
    }

    // ✅ Cleanup
    if (pdf.cleanup) pdf.cleanup(); // PDF.js internal cleanup
    if (pdf.destroy) await pdf.destroy(); // Fully dispose PDF memory
    pdf.destroy?.(); // optional: cleanup loading task
    typedArray = null;
    file = null;
    pageNum = 0;
}
...
async function processPageBatch(pdf, start, end, _selectedLang, fileID, file) {
    const promises = [];

    for (let i = start; i <= end; i++) {
        const promise = pdf.getPage(i).then(async (page) => {
            const text = await performPageOCR(page, _selectedLang, file);
            await storePageText(fileID, i, text);
            return text;
        });
...
async function performPageOCR(page, _selectedLang, file) {
    const scale = 2.5;
    ...
    const result = await Tesseract.recognize(canvas, _selectedLang, {
        logger: (m) => {
        ...
        tessedit_create_hocr: 1,   // Include layout information
        tessedit_create_txt: 1,    // Include plain text
        tessedit_create_box: 1     // Include bounding box information
    });
    // Clean canvas
          ...
          <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
              ...
              <div class="row text-center" id="_loading${fileIndex}">
                ...
                <p class="text-danger d-none fetchPages${fileIndex}">Fetching pages of selected pdf</p>
              </div>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <label for="ocrLang${fileIndex}" class="form-label">Select OCR language for this PDF:</label><br/>
...
async function downloadAllTextFiles() {
    ...
    try {
        for (let file of selectedFiles) {
            if (filesToIgnore.includes(file.index)) continue;

            const textFileName = file.name.replace(/\.[^/.]+$/, '.txt');
            const extractedText = await retrieveStoredText(file.index);
...
function cleanUpCanvas(canvas) {
    ...
    // Done – no need to mess with context properties
}